...
*   **3. The Data Foundation (dbt & PostgreSQL):**
    ...
    *   **"เส้นทางแรก: Structured Data แบบ Modern ELT"**
        ...
        *   **"3. `marts` (Business Logic & Star Schema)"**
            ...
            *(Note for Jinnawat: Switch to the "Star Schema (Marts Layer)" ER Diagram here.)*
            *   **อธิบายโครงสร้าง Star Schema เพิ่มเติม:** "ถ้าเรามาดูหน้าตาของ Star Schema ที่ผมออกแบบไว้ (อ้างอิงจาก Diagram) ผมวางตาราง `fact_tickets` ไว้ตรงกลางเพื่อเก็บ Event การเปิดตั๋วครับ โดยผมได้คำนวณ **Derived Metric** อย่าง `resolution_time_hours` เตรียมไว้ให้เลย เพื่อลดภาระของ AI เวลาต้องมานั่งบวกลบเวลาเองครับ
            ส่วนพวกรายละเอียดผมแยกออกไปเป็นตาราง Dimension เช่น `dim_products` และ `dim_customers` ครับ ไฮไลท์สำคัญคือใน `dim_customers` ผมจัดการเรื่อง **PII Masking ตั้งแต่ระดับ Database** เลยครับ เช่น อีเมลกับเบอร์โทร ผม Hash ด้วย SHA-256 ทิ้งไปเลย และวันเกิด (DOB) ผมแปลงให้เป็นแค่อายุ (`age_years`) แทน เพื่อความปลอดภัยสูงสุด นอกจากนี้ผมยังรองรับการเก็บประวัติข้อมูลลูกค้าแบบ **SCD Type 2** เผื่อกรณีที่ลูกค้ามีการเปลี่ยน Segment ด้วยครับ"
